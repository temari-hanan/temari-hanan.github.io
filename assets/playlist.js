...
function renderSongList() {
  var artistQuery = $('#searchArtist').val().toLowerCase();
  var songQuery = $('#searchSong').val().toLowerCase();
  var $songsDiv = $('#songs');
  $songsDiv.empty();
  availableSongs.forEach(function(song, index) {
    var matchArtist = song.artist.toLowerCase().indexOf(artistQuery) !== -1;
    var matchSong = song.song.toLowerCase().indexOf(songQuery) !== -1;
    if (artistQuery && !matchArtist) return;
    if (songQuery && !matchSong) return;

    var checkboxId = 'song_checkbox_' + index;
    var $item = $('<div class="song-item"></div>');
    var $checkbox = $('<input type="checkbox" id="' + checkboxId + '" class="song-checkbox" data-index="' + index + '">');
